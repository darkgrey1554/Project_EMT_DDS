...
namespace atech
{
  namespace Process
  {
    ...
    unsigned long GetProcessMemory()
    {
      ...
      SIZE_T msize = pmc.WorkingSetSize;
      return(static_cast<unsigned long>(msize));
    }

    int StartProcess(const char* proc_name_, const char* proc_args_, const char* working_dir_, const bool create_console_, const scd_Process_StartMode process_mode_, const bool block_)
    {
      int ret_pid = 0;
...
namespace
{
  ...
  std::string getTerminalEmulatorCommand()
  {
    ...
    if (pipe)
    {
      ...
      process_stub_output = AtechUtils::String::Trim(process_stub_output);
  ...
  namespace Process
  {
    ...
    */
    ...
    int StartProcess(const char* proc_name_,
      ...
      const scd_Process_StartMode  process_mode_,
      const bool  block_)
    {
      ...
      if (process_mode_ != scd_Process_StartMode::proc_smode_hidden)
      {
      ...
      else if (child_pid == 0)
      {
        ...
        else
        {
          ...
          std::vector<std::string> proc_argv = AtechUtils::CommandLine::ToArgv(std::string(proc_args_));
          ...
          {
            std::vector<std::string> terminal_emulator_argv = AtechUtils::CommandLine::ToArgv(std::string(terminal_emulator_command));
