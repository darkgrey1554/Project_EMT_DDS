...
/**
* @brief  scd process interface
**/

//#include <ecal/ecal.h>
//
#include "../include/mon/scd_process.h"
//#include "ecal_def.h"
...
#include "scd_process_stub.h"
#include "../../scd_utils/include/scd_utils/scd_utils.h"
...
namespace atech
{
  namespace Process
  {
    ...
    void DumpConfig(std::string& cfg_s_)
    {
     ...
     /* if (!atech::IsInitialized())
      {
        sstream << "Components               : NOT INITIALIZED ( call atech::Initialize() )";
        sstream << std::endl;
        cfg_s_ = sstream.str();
        return;
      }*/
     ...
     /* sstream << "Default INI              : " << g_default_ini_file << std::endl;*/ // WARNING: The scd Recorder relies on the identifier "Default INI" to obtain the ecal.ini path (It parses the output of this function)
      sstream << std::endl;

      sstream << "------------------------- NETWORK --------------------------------" << std::endl;
      sstream << "Host name                : " << Process::GetHostName() << std::endl;
      sstream << "Host id                  : " << Process::GetHostID() << std::endl;
      /*if (scdPAR(NET, ENABLED))
      {
      ...
      sstream << "Network ttl              : " << scdPAR(NET, UDP_MULTICAST_TTL) << std::endl;
      sstream << "Network sndbuf           : " << GetBufferStr(scdPAR(NET, UDP_MULTICAST_SNDBUF)) << std::endl;
      sstream << "Network rcvbuf           : " << GetBufferStr(scdPAR(NET, UDP_MULTICAST_RCVBUF)) << std::endl;
      sstream << "Multicast group          : " << scdPAR(NET, UDP_MULTICAST_GROUP) << std::endl;
      sstream << "Multicast mask           : " << scdPAR(NET, UDP_MULTICAST_MASK) << std::endl;
      int port = scdPAR(NET, UDP_MULTICAST_PORT);
      sstream << "Multicast ports          : " << port << " - " << port + 10 << std::endl;
      auto bandwidth = scdPAR(NET, BANDWIDTH_MAX_UDP);*/
     /* if (bandwidth < 0)
      {
      ...
      sstream << "Synchronization realtime : " << scdPAR(TIME, SYNC_MOD_RT) << std::endl;
      sstream << "Synchronization replay   : " << scdPAR(TIME, SYNC_MOD_REPLAY) << std::endl;
      sstream << "State                    : ";*/
    ...
    /*  sstream << "Layer Mode INPROC  (scd)     : " << LayerMode(scdPAR(PUB, USE_INPROC))  << std::endl;
      sstream << "Layer Mode SHM     (scd)     : " << LayerMode(scdPAR(PUB, USE_SHM))     << std::endl;
      sstream << "Layer Mode UDP MC  (scd)     : " << LayerMode(scdPAR(PUB, USE_UDP_MC))  << std::endl;
      sstream << std::endl;*/

      sstream << "------------------------- SUBSCRIPTION LAYER DEFAULTS ------------"               << std::endl;
     /* sstream << "Layer Mode INPROC  (scd)     : " << LayerMode(scdPAR(NET, INPROC_REC_ENABLED))  << std::endl;
      sstream << "Layer Mode SHM     (scd)     : " << LayerMode(scdPAR(NET, SHM_REC_ENABLED))     << std::endl;
      sstream << "Layer Mode UDP MC  (scd)     : " << LayerMode(scdPAR(NET, UDP_MC_REC_ENABLED))  << std::endl;
      sstream << "Npcap UDP Reciever            : " << LayerMode(scdPAR(NET, NPCAP_ENABLED));*/
#ifdef ECAL_NPCAP_SUPPORT
      if(scdPAR(NET, NPCAP_ENABLED) && !Udpcap::Initialize())
      {
        sstream << " (Init FAILED!)";
      }
#endif // ECAL_NPCAP_SUPPORT
...
namespace
{
  void create_proc_id()
  {
    if (atech::g_process_id == 0)
    {
      atech::g_process_id   = GetCurrentProcessId();
      atech::g_process_id_s = std::to_string(atech::g_process_id);
    }
  }
}
...
namespace atech
{
  namespace Process
  {
    ...
    std::string GetProcessName()
    {
      ...
      {
        char pname[1024] = { 0 };
        GetModuleFileNameExA(GetCurrentProcess(), 0, pname, 1024);
    ...
    int StartProcess(const char* proc_name_, const char* proc_args_, const char* working_dir_, const bool create_console_, const scd_Process_eStartMode process_mode_, const bool block_)
    {
      int ret_pid = 0;
...
namespace
{
  ...
  std::string getTerminalEmulatorCommand()
  {
    ...
    const std::string terminal_emulator_command ="";//scdPAR(PROCESS, TERMINAL_EMULATOR);
    ...
    if (pipe)
    {
      ...
      process_stub_output = ScdUtils::String::Trim(process_stub_output);
      ...
      if (process_stub_output != std::string(ECAL_PROCESS_STUB_VERSION_STRING))
      {
        std::cerr << "[PID " << getpid() << "]: " << "Error testing for ecal_process_stub: Got faulty version string: \"" << process_stub_output << "\". "
                  << "Maybe scd is not installed correctly. Switching to fallback mode." << std::endl;
        return "";
      }
  ...
  namespace Process
  {
    ...
    */
    ...
    int StartProcess(const char* proc_name_,
      ...
      const scd_Process_eStartMode  process_mode_,
      const bool  block_)
    {
      ...
      if (process_mode_ != scd_Process_eStartMode::proc_smode_hidden)
      {
      ...
      else if (child_pid == 0)
      {
        ...
        else
        {
          ...
          std::vector<std::string> proc_argv = ScdUtils::CommandLine::ToArgv(std::string(proc_args_));
          ...
          {
            std::vector<std::string> terminal_emulator_argv = ScdUtils::CommandLine::ToArgv(std::string(terminal_emulator_command));
          ...
          // Open FIFO and send the current PID to the scd. The main process
