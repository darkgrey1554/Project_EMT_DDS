...
namespace nlohmann {
    ...
    template <typename T> struct adl_serializer<std::shared_ptr<T>> {
        ...
        static void to_json(ordered_json& j, const std::shared_ptr<T>& ptr) {
            ...
            if (typeid(atech::common::Config) == typeid(T))
            {
                if (std::dynamic_pointer_cast<atech::common::UaConfig>(ptr))
                {
                    j = *std::dynamic_pointer_cast<atech::common::UaConfig>(ptr);
                }
                else if (std::dynamic_pointer_cast<atech::common::DdsConfig>(ptr))
                {
                    j = *std::dynamic_pointer_cast<atech::common::DdsConfig>(ptr);
                }
                else if (std::dynamic_pointer_cast<atech::common::SmConfig>(ptr))
                {
                    j = *std::dynamic_pointer_cast<atech::common::SmConfig>(ptr);
                }
                else if (std::dynamic_pointer_cast<atech::common::WsConfig>(ptr))
                {
                    j = *std::dynamic_pointer_cast<atech::common::WsConfig>(ptr);
                }
            }
            else if (ptr.get()) {
                j = *ptr;
